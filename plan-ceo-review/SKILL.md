...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only use direct user question in chat when there is a genuine decision with meaningful tradeoffs.
...
### TODOS.md updates
Present each potential TODO as its own individual direct user question in chat. Never batch TODOs — one per question. Never silently skip this step.
...
Identify at least 5 "bonus chunk" opportunities (<30 min each) that would make users think "oh nice, they thought of that." Present each delight opportunity as its own individual direct user question in chat. Never batch them. For each one, describe what it is, why it would delight users, and effort estimate. Then present options: **A)** Add to TODOS.md as a vision item **B)** Skip **C)** Build it now in this PR.
