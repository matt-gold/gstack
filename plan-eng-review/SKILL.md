...
3. **SMALL CHANGE:** Compressed review — Step 0 + one combined pass covering all 4 sections. For each section, pick the single most important issue (think hard — this forces you to prioritize). Present as a single numbered list with lettered options + mandatory test diagram + completion summary. Use one round of direct user questions at the end. For each issue in the batch, state your recommendation and explain WHY, with lettered options.
...
**STOP.** For each issue found in this section, ask the user directly in chat. One issue per question. Present options, state your recommendation, explain WHY. Do NOT batch multiple issues into one question. Only proceed to the next section after ALL issues in this section are resolved.
...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in AGENTS.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then ask the user directly in chat to confirm the eval scope.
...
* **Escape hatch:** If a section has no issues, say so and move on. If an issue has an obvious fix with no real alternatives, state what you'll do and move on — don't waste a question on it. Only ask direct user questions when there is a genuine decision with meaningful tradeoffs.
...
* When using direct user questions, label each option with issue NUMBER and option LETTER so I don't get confused.
